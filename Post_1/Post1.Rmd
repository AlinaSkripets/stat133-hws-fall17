...
All done. Dplyr package really makes our life easier. 
On to the next task.
...
Our data starts with August, 2015 which is when the Dang Foods startup started recording their sales. It ends with September 2016 and the rest of the year is up to us to forecast. 
```{r}
...
We can also graph a time series using season plot. On the graph below we see that sales for Aug 16 and Aug 15 differ drastically. This indicates to me that even if we forecast the sales for a few months of 2016, it is hardly a good prediction of how the business is going to develop. 
```{r}
...
Hyndman's package offers a way to estimate accuracy of our forecast, as expalained in more detains in his [presentation](https://www.r-bloggers.com/time-series-analysis-using-r-forecast-package/). The syntax is straight forward. We just need to indicate the  object of type forecast.
...
R package called forecasting offers many useful tools to analyze and predict time trends. However, we need to keep in mind limitations of our data and not reply heavily on the forecasts made but rather use them as guidelines to the likely events.
...
[Forecasting time series - comments and suggestions](https://www.statmethods.net/advstats/timeseries.html)
[Reference Manual for R forecast package](https://cran.r-project.org/web/packages/forecast/forecast.pdf)